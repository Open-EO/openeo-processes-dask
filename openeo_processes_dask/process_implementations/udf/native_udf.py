...
class NativeUdfProcessor:
    ...
    def run_udf(self, data: da.Array, udf: str, runtime: str, 
        ...
        if all(str(dim).startswith('dim_') for dim in xr_data.dims):
            ...
            _log.warning(f"Generic dimensions detected for {ndim}D data: {list(xr_data.dims)}")
    ...
    def _execute_udf_native(self, datacube: xr.DataArray, udf_code: str, 
                           context: Dict[str, Any]) -> xr.DataArray:
    ...
    def _find_datacube_function(self, udf_namespace: Dict[str, Any]) -> Callable:
        ...
        user_functions = [
            (name, func) for name, func in udf_namespace.items() 
            if callable(func) and not name.startswith('_') and name not in self._allowed_imports
        ]
...
def run_udf(data: da.Array, udf: str, runtime: str, 
           context: Optional[dict] = None) -> xr.DataArray:
    """
