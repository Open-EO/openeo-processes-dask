...
def aggregate_temporal(
    ...
    data = data.groupby_bins(
        group=kwargs["dimension"], bins=intervals, labels=labels
    )
...
def aggregate_temporal_period(
    ...
    periods_to_frequency = {'hour': 'H', 'day': 'D', 'week': 'W', 'month': 'M', 'season': 'QS-DEC', 'year': 'AS'}
...
def aggregate_spatial(
    data: RasterCube, geometries: VectorCube, reducer: Callable, target_dimension: str = "result", **kwargs
) -> VectorCube:
    if len(data.dims) > 3:
    ...
    for _, row in geometries.iterrows():
        mask = geometry_mask([Geometry(row["geometry"], crs=geometries.crs)], data.geobox, invert=True)
    ...
    xr_crop_list_reduced = reducer(data=reducer(data=xr_crop_list, dimension="x"), dimension="y")
